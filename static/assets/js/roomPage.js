...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Replace the old Add Light logic with modal logic
    document.getElementById('btn-add-light').addEventListener('click', () => {
        // Show the modal for adding a new light
        document.getElementById('modal-add-light').style.display = 'block';
    });

    // Modal close logic for add light modal
    document.querySelectorAll('.close-modal').forEach(btn => {
        btn.addEventListener('click', () => {
            document.getElementById('modal-add-light').style.display = 'none';
        });
    });

    window.addEventListener('click', (e) => {
        if (e.target === document.getElementById('modal-add-light')) {
            document.getElementById('modal-add-light').style.display = 'none';
        }
    });

    // Save new light from modal
    document.getElementById('save-light').addEventListener('click', async () => {
        const name = document.getElementById('light-name').value.trim();
        const port = document.getElementById('light-port') ? document.getElementById('light-port').value.trim() : null;

        if (!name) {
            alert('Please enter a light name.');
            return;
        }
        ...
        try {
            ...
            if (document.getElementById('light-port')) document.getElementById('light-port').value = '';
        } catch (error) {
    ...
    try {
        const response = await makeApiCall(`/api/rooms/${roomId}/thermostat`, 'GET');
        document.getElementById('targeted-temperature').textContent = response.target_temperature;
        targetTemperature = parseInt(response.target_temperature);
    } catch (error) {
        console.error('Failed to load thermostat', error);
    }
}
...
// Add this after your other modal logic
