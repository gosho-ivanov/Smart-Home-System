...
class SmartHomeDB:
    ...
    # Room management
    def create_room(self, user_id, name, thermostat_port):
        ...
        try:
            ...
            device_query = """
            INSERT INTO devices (room_id, name, type, status, port)
            VALUES (%s, %s, 'thermostat', 'on', %s)
            """
            cursor.execute(device_query, (room_id, f"{name} Thermostat", thermostat_port))
            device_id = cursor.lastrowid

            # Add thermostat data
            thermostat_query = """
            INSERT INTO thermostats (thermostat_id, current_temperature, target_temperature,thermostat_port)
            VALUES (%s, %s, %s, %s)
            """
            cursor.execute(thermostat_query, (device_id, 22.0, 24.0, thermostat_port))
    ...
    def add_device(self, room_id, name, device_type, port, status='off', brightness=None,):
        cursor = None
        try:
            cursor = self.connection.cursor()
            query = """
            ...
            VALUES (%s, %s, %s, %s, %s)
            """
            cursor.execute(query, (room_id, name, device_type, status, port))
            device_id = cursor.lastrowid

            if device_type == 'light':
                brightness = brightness or 100
                self.execute_query(
                    "INSERT INTO lights (light_id, brightness, light_port) VALUES (%s, %s, %s)",
                    (device_id, brightness, port)
                )
